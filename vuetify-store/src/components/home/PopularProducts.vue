<script setup>
import { useProductsStore } from '@/stores/ProductsStore'
import VerticalProduct from '../cards/VerticalProduct.vue'

// store
const productsStore = useProductsStore()

// data

// methods
function addToCart(index, quantity = 1) {
  productsStore.addItemToCart({ itemId: index, quantity: quantity })
  productsStore.updateSnackBar({ show: true })
}
</script>

<template>
  <v-container mb-12>
    <!-- Title row -->
    <v-row>
      <v-col sm="10" offset-sm="1" md="8" offset-md="2">
        <h2>
          Popular Products
          <v-btn variant="text" color="primary" small to="store" :ripple="false">View All</v-btn>
        </h2>
      </v-col>
    </v-row>

    <!-- Products row -->
    <v-row>
      <v-col sm="10" offset-sm="1" lg="8" offset-lg="2">
        <v-row>
          <v-col
            sm="6"
            md="4"
            v-for="product in productsStore.products.slice(0, 3)"
            :key="product.name"
          >
            <VerticalProduct :product="product" :addToCart="addToCart" />
          </v-col>
        </v-row>
      </v-col>
    </v-row>
  </v-container>
</template>
